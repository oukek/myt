...
class MytRpc(object):
        ...
        if self._handle > 0:
            ...
            if ptr:
                try:
                    ...
                    base64_data = base64.b64encode(bytearray(buf)).decode('utf-8')
                    # 添加base64前缀
                    prefix = "data:image/png;base64," if type == 0 else "data:image/jpeg;base64,"
                    ret = prefix + base64_data
                finally:
                    # 释放指针内存
                    self._rpc.freeRpcPtr.argtypes = [ctypes.c_void_p]
                    self._rpc.freeRpcPtr(ptr)
    ...
    def screentshotEx(self,left, top, right, bottom, type, quality, file_path):
        ret = False
        base64_data = self.takeCaptrueCompressEx(left, top, right, bottom, type, quality) 
    ...
    def screentshot(self, type, quality, file_path):
        ret = False
        base64_data = self.takeCaptrueCompress(type, quality) 
        if base64_data != False:
            # 移除base64前缀
            if type == 0:
                prefix = "data:image/png;base64,"
